...
//   3. 把两者合并成 {文件: {规则: [违规数值]}},与 JSON 基线做"只准变好"的棘轮比对。
//
// 用法:
...
// 基线、参数错)。判定不了的时候一定要炸,不能当成零违规 —— 那是用绿色掩盖问题。
...
const BASELINE_COMMENT =
  "代码坏味道门禁的存量基线,由 easyui-check-smells --update 生成。阈值见 eslint.smells.config.mjs " +
  "(eslint 规则)与 scripts/smells-core.mjs(文件规模)。这份文件只应该变小。";
...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (pending === "targets" && !arg.startsWith("--")) {
...
async function runEslint(options) {
  ...
  return eslint.lintFiles(options.targets);
}
...
function updateBaseline(baselinePath, found, options) {
  ...
  if (existsSync(baselinePath)) {
    const { regressions } = compareBaseline(found, loadBaseline(baselinePath));
  ...
  console.log(`基线已更新:${Object.keys(found).length} 个文件 / ${total} 条存量违规 -> ${options.baseline}`);
  return 0;
}
...
async function main(argv) {
  ...
  const { regressions, improvements } = compareBaseline(found, loadBaseline(baselinePath));
  if (regressions.length > 0) {
    reportRegressions(regressions);
    return 1;
  }
  ...
  console.log(
    `代码坏味道门禁通过:${Object.keys(found).length} 个文件 / ${countViolations(found)} 条存量违规,未超基线。`,
  );
