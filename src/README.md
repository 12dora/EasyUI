...
| Accents  | `--amber` (primary/blue) + `--amber-soft`, `--signal` (danger/red fills, icons, required marks) + `--signal-ink` (red **text**: errors, notices, status words), `--bond` (indigo — the one accent that also carries its own text: 5.58:1 on its own 8% tint, so it has no `-ink` sibling), `--evergreen` (success fills, borders, icon glyphs) + `--evergreen-ink` (green **text**: success notices, `Badge tone="evergreen"` — the fill green is 3.77:1 and fails AA), `--plum` (fuchsia #C026D3, categorical — no status meaning, e.g. the 多选 question-type tag) + `--plum-ink` (#A21CAF, plum **text**: `Badge tone="plum"` — the fill is 4.18:1 on its own 8% tint and fails AA) |
