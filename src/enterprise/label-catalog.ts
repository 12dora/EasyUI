    ...
    return {
      ...
      access: {
        ...
        configuration: {
          ...
          authentikApiBaseUrl: "Authentik API 地址",
          authentikApiToken: "Authentik API token",
          easyAuthTitle: "EasyAuth 配置",
          easyAuthDescription: "配置权限服务 authority、应用凭据与权限申请入口。",
          baseUrl: "EasyAuth 地址",
          appKey: "App Key",
          advancedTitle: "高级配置",
          advancedDescription: "OIDC 高级地址与可用的目录同步能力。",
        },
      },
      upstream: {
        ...catalog.upstream,
  ...
  return {
    ...
    access: {
      ...
      configuration: {
        ...
        jwksUri: "JWKS URI",
        userinfoEndpoint: "UserInfo endpoint",
        authentikApiBaseUrl: "Authentik API URL",
        authentikApiToken: "Authentik API token",
...
function chineseCatalog(brand: EnterpriseCatalogBrand): EnterpriseStaticLabelCatalog {
  ...
  return {
    ...
    access: {
      ...
      configuration: { loading: "正在加载配置", loadFailed: "配置加载失败", saveFailed: "配置保存失败", saved: "配置已保存", save: "保存", enabled: "启用工作账号登录", configured: "已配置（不回显）", notConfigured: "未配置", clearSecret: "清除已保存凭据", authorityHint: "留空会保留现有凭据；更改服务地址会清除旧凭据。", oidcTitle: "工作账号登录配置", oidcDescription: "配置工作账号登录与回调地址。", issuer: "登录服务地址", clientId: "应用标识", clientSecret: "应用密钥", scopes: "授权范围", authorizationEndpoint: "授权地址", tokenEndpoint: "令牌地址", jwksUri: "密钥集地址", userinfoEndpoint: "用户信息地址", redirectBaseUrl: "回调站点地址", redirectUri: "回调地址", frontendBaseUrl: "前端地址", serverBaseUrl: "服务端地址", authentikApiBaseUrl: "身份管理 API 地址", authentikApiToken: "身份管理 API 令牌", easyAuthTitle: "权限服务配置", easyAuthDescription: "配置权限服务地址、应用凭据与权限申请入口。", baseUrl: "权限服务地址", appKey: "应用标识", credential: "应用凭据", permissionRequestUrl: "权限申请地址", discover: "自动发现", connectionTest: "连接测试", userSync: "立即同步用户", userSyncEnabled: "启用用户同步", userSyncInterval: "同步间隔（分钟）", advancedTitle: "高级管理员配置", advancedDescription: "协议端点与目录同步等高级项，仅管理员需要。", advancedShow: "展开", advancedHide: "收起", operationSucceeded: "操作成功", operationFailed: "操作失败", guideAriaLabel: "查看 {field} 说明", guides: {} },
