...
function chineseCatalog(brand: EnterpriseCatalogBrand): EnterpriseStaticLabelCatalog {
  ...
  return {
    ...
    access: {
      page: { title: "登录与权限", tabsAriaLabel: "登录与权限", description: "管理登录方式与业务权限。", loginTab: "工作账号登录", permissionsTab: "业务权限", authentikTitle: "工作账号登录", authentikDescription: "公司统一身份登录服务。", easyAuthTitle: "权限服务", easyAuthDescription: "业务权限、数据范围与菜单授权。", configured: "已配置", notConfigured: "未配置", endpoint: "服务地址", lastChecked: "最近检查" },
      configuration: { loading: "正在加载配置", loadFailed: "配置加载失败", saveFailed: "配置保存失败", saved: "配置已保存", save: "保存", enabled: "启用工作账号登录", configured: "已配置（不回显）", notConfigured: "未配置", clearSecret: "清除已保存凭据", authorityHint: "留空会保留现有凭据；更改服务地址会清除旧凭据。", oidcTitle: "工作账号登录配置", oidcDescription: "配置工作账号登录与回调地址。", issuer: "登录服务地址", clientId: "应用标识", clientSecret: "应用密钥", scopes: "授权范围", scopesHint: "登录时向身份服务申请的信息范围。", scopeOptions: { openid: "基础身份（必需）", profile: "姓名与用户名", email: "邮箱", dingtalk: "钉钉用户标识" }, authorizationEndpoint: "授权地址", tokenEndpoint: "令牌地址", jwksUri: "密钥集地址", userinfoEndpoint: "用户信息地址", redirectBaseUrl: "回调站点地址", redirectUri: "回调地址", frontendBaseUrl: "前端地址", serverBaseUrl: "服务端地址", easyAuthTitle: "权限服务配置", easyAuthDescription: "配置权限服务地址、应用凭据与权限申请入口。", baseUrl: "权限服务地址", appKey: "应用标识", credential: "应用凭据", webhookSecret: "Webhook 密钥", webhookSecretHint: "须与权限服务的事件密钥一致。", permissionRequestUrl: "权限申请地址", discover: "自动发现", connectionTest: "测试连接", advancedTitle: "高级管理员配置", advancedDescription: "协议端点等高级项，仅管理员需要。", advancedShow: "展开", advancedHide: "收起", operationSucceeded: "操作成功", operationFailed: "操作失败", retry: "重试", guideAriaLabel: "查看 {field} 说明", guides: {} },
      authorization: authorizationChinese(close, loading),
      directory: {
        title: "用户目录同步", description: "从用户目录同步人员名单。",
        ...
        lastSyncTitle: "最近一次同步", lastSyncNever: "尚未同步。", lastSyncAt: "同步时间",
        statusLabels: { completed: "已完成", not_authoritative: "数据不完整，未写入", drift: "目录已变更，未写入", failed: "失败", not_configured: "未配置" },
        statusExplanations: {
          completed: "已完整读取目录并应用变更。",
          not_authoritative: "目录数据不完整或已过期，本次未写入任何变更。",
          drift: "读取期间目录发生变更，本次结果已放弃，稍后自动重试。",
          failed: "同步未完成，本地数据保持不变，详见下方错误信息。",
          not_configured: "尚未配置用户目录，不会自动同步人员。",
        },
        trustAuthoritative: "本次数据完整，目录中已移除的人员已在本系统停用。",
        trustNotAuthoritative: "本次数据不完整，未停用任何人员；以下计数仅表示读取结果。",
        trustUnchanged: "本次未写入任何变更，也未停用任何人员。",
        trustNotConfigured: "尚未执行同步，本地数据保持不变。",
        incomplete: "目录未读取完整（分页未取完或部分来源不可用）。",
        stale: "目录数据已过期。",
        counts: { upstreamTotal: "目录人数", created: "新增", updated: "更新", deactivated: "停用", unmapped: "无登录账号" },
        unmappedHint: "“无登录账号”指目录中没有对应登录身份的人员：已建档，可担任负责人或审批人，但无法登录本系统。",
  ...
  return {
    ...
    access: {
      ...
      directory: {
        ...
        lastSyncTitle: "Last sync", lastSyncNever: "Not synced yet.", lastSyncAt: "Synced at",
        statusLabels: { completed: "Completed", not_authoritative: "Incomplete data, not written", drift: "Directory changed, not written", failed: "Failed", not_configured: "Not configured" },
        statusExplanations: {
          completed: "The directory was read in full and the changes were applied.",
          not_authoritative: "Directory data was incomplete or out of date, so no changes were written.",
          drift: "The directory changed while it was being read, so this run was discarded. It will be retried automatically.",
          failed: "The sync did not finish. Local data is unchanged — see the error below.",
          not_configured: "No user directory is configured, so nobody is synced automatically.",
        },
        trustAuthoritative: "This run had complete data, so people removed from the directory were deactivated here.",
        trustNotAuthoritative: "This run had incomplete data, so nobody was deactivated. The counts below describe what was read, not what was written.",
        trustUnchanged: "No changes were written and nobody was deactivated.",
        trustNotConfigured: "No sync has run yet, so local data is unchanged.",
        incomplete: "The directory was not read in full (pages missing, or a source was unavailable).",
        ...
        unmappedHint: "\u201cNo sign-in account\u201d means directory people with no matching sign-in identity: they are recorded and can own or approve work, but cannot sign in.",
...
function appearanceSettingsChinese(retry: string): EnterpriseAppearanceSettingsLabels {
  return { title: "外观", description: "调整界面的显示效果。", visualTitle: "视觉效果", tableDensity: "表格", rowSpacing: "行距", densityCompact: "紧凑", densityComfortable: "宽松", saving: "正在保存", saveFailed: "外观设置保存失败，请重试", showFooter: "显示页脚", globalLoadFailed: "全局外观设置加载失败", retry, globalSaved: "全局外观设置已保存", globalSaveFailed: "全局外观设置保存失败，请重试" };
}
